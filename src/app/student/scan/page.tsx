...
export default function ScanQRPage() {
    ...
    try {
      ...
      await scanner.start(
        ...
        (decodedText: string) => {
          ...
          stopScanner().then(() => handleQRResult(decodedText));
        },
        () => {}
      );
    } catch (err: any) {
  ...
  const handleQRResult = async (data: string) => {
    let activityId = data.trim();

    if (activityId.startsWith('http')) {
      try { activityId = new URL(activityId).searchParams.get('id') || activityId; } catch {}
    }
    if (activityId.startsWith('{')) {
      try {
        const parsed = JSON.parse(activityId);
        activityId = parsed.activityId || parsed.id || activityId;
      } catch {}
    }
